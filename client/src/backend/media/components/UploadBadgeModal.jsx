...
// ── Main Modal ─────────────────────────────────────────────────────────────
export default function UploadBadgeModal({ isOpen, onClose, onUploaded, storageTargetLabel = "Cloudinary CDN" }) {
  ...
  async function handleSubmit(e) {
    ...
    setGlobalError("");
    try {
      const response = await uploadMediaFiles(validFiles, "badges");
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Upload Badge files"
      >
        {/* Header */}
        <div className="umm-header">
          <div className="umm-header__info">
            <h3>Upload Badge</h3>
            <p>Images only. Max 5 MB per file.</p>
          </div>
          <button
            type="button"
            className="umm-close"
